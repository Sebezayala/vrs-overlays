...
;(L.Control.GroupedLayers = L.Control.extend({
  ...
  removeLayer: function (a) {
    ...
    var c = this._getLayer(b)
    return (
      c && delete this._layers[this._layers.indexOf(c)], this._update(), this
    )
  },
  ...
  _initLayout: function () {
    ...
    var b = (this._container = L.DomUtil.create('div', a))
    ...
    if (this.options.collapsed) {
      L.Browser.android ||
        L.DomEvent.on(b, 'mouseover', this._expand, this).on(
          b,
          'mouseout',
          this._collapse,
          this
      ...
      (d.title = 'Layers'),
      L.Browser.touch
        ? L.DomEvent.on(d, 'click', L.DomEvent.stop).on(
          d,
          'click',
          this._expand,
          this
        )
        : L.DomEvent.on(d, 'focus', this._expand, this),
  ...
  _onLayerChange: function (a) {
    ...
    var c = this._getLayer(L.Util.stamp(a.layer))
    c &&
  ...
  _addItem: function (a) {
    ...
    var f = this._map.hasLayer(a.layer)
    a.overlay
      ? a.group.exclusive
        ? ((d = 'leaflet-exclusive-group-layer-' + a.group.id),
        (b = this._createRadioElement(d, f)))
    ...
    if (
      ...
      if (!h) {
        ...
        (h.className = 'leaflet-control-layers-group'),
        (h.id = 'leaflet-control-layers-group-' + a.group.id)
        var i = document.createElement('label')
        if (
          ((i.className = 'leaflet-control-layers-group-label'),
          a.group.name !== '' &&
            !a.group.exclusive &&
            this.options.groupCheckboxes)
        ) {
          var j = document.createElement('input')
          ;(j.type = 'checkbox'),
          (j.className = 'leaflet-control-layers-group-selector'),
          (j.groupID = a.group.id),
          (j.legend = this),
          L.DomEvent.on(j, 'click', this._onGroupInputClick, j),
          i.appendChild(j)
        }
        var k = document.createElement('span')
        ;(k.className = 'leaflet-control-layers-group-name'),
        (k.innerHTML = a.group.name),
        i.appendChild(k),
        h.appendChild(i),
        c.appendChild(h),
        (this._domGroups[a.group.id] = h)
      }
      c = h
  ...
  _onGroupInputClick: function () {
    ...
    for (a = 0; f > a; a++) {
      ...
      b.groupID === this.groupID &&
          ...
          (c = d._getLayer(b.layerId)),
          b.checked && !d._map.hasLayer(c.layer)
            ? d._map.addLayer(c.layer)
            : !b.checked &&
              d._map.hasLayer(c.layer) &&
              d._map.removeLayer(c.layer))
    }
    d._handlingClick = !1
  },
